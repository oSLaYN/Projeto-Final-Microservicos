...
routes.post("/updateUser", async (req, res) => {
    try {
        const { id, username, fullname, password, type } = req.body;
        if (!id || !username || !fullname || !password || !type) { return res.status(400).json({ message: "Dados em falta." }); }
        try {
            const isAuthorized = await fetch('http://10.96.18.2/api/auth/isAuthorized');
            if (!isAuthorized.ok) { return res.status(400).json({ message: "Sessão Inválida, Sem Acesso." });}
            if (req.user.type != "admin") { return res.status(400).json({ message: "Acesso Negado, Sem Acesso." }); }
            try {            
                const results = await sqlQuery("UPDATE users SET username=?, fullname=?, password=?, type=? WHERE id=?", [username, fullname, password, type, id]);
                return res.status(200).json({ message: "Utilizador criado com sucesso!" });
            } catch (error) {
                return res.status(500).json({ message: "Ocorreu um erro: " + error });
            }
