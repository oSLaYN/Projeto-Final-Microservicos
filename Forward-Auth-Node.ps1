...
$service = kubectl get service auth-node -n auth-node --no-headers -o custom-columns=":metadata.name" 2>$null

if (-not $service) {
    Write-Host "`n[ERRO] Serviço 'auth-node' não encontrado. Verifique se o serviço está correto."
    exit
}
